...
namespace Watchables
{
	...
	/// <summary>
	/// Generic type extension base of <see cref="WatchableBase"/>. Implements <see cref="IWatchable{T}"/>. 
	/// Provides implicit casting to <typeparamref name="T"/> and access to a wrapper <see cref="ReadOnlyWatchable{T}"/>.
	/// </summary>
	public abstract class WatchableBase<T> : WatchableBase, IWatchable<T>
	{

		protected ReadOnlyWatchable<T> _readOnlyWrapper;
		protected T _value;

		/// <summary> A read-only version of this <see cref="WatchableBase{T}"/>. </summary>
		public ReadOnlyWatchable<T> ReadOnlyWrapper => _readOnlyWrapper ??= new(this);

		public static implicit operator T(WatchableBase<T> watchable) => watchable.ToValue();
