...
namespace Watchables
{
  public sealed class OpChainStep<T> where T : unmanaged
  {
    ...
    /// <param name="fatal"> If the value's destruction is fatal to the chain. </param>
    public static OpChainStep<T> Subtract(IValueWrapper<T> value, bool fatal = false) => new(OperationType.Subtract, value, fatal);
    ...
    public static OpChainStep<T> Round() => new(OperationType.Round, false);
    /// <summary> Create a new Floor step. Unsupported for integral types. </summary>
    public static OpChainStep<T> Floor() => new(OperationType.Floor, false);
    /// <summary> Create a new Ceiling step. Unsupported for integral types. </summary>
    public static OpChainStep<T> Ceiling() => new(OperationType.Ceiling, false);
    /// <summary> Create a new Truncate step. Unsupported for integral types. </summary>
    public static OpChainStep<T> Truncate() => new(OperationType.Truncate, false);
    /// <summary> Create a new Absolute Value step. </summary>
    public static OpChainStep<T> Absolute() => new(OperationType.Absolute, false);
    /// <summary> Create a new step forcing the chain's value to be negative. </summary>
    public static OpChainStep<T> AsNegative() => new(OperationType.AsNegative, false);
    /// <summary> Create a new step flipping the chain's sign. </summary>
    public static OpChainStep<T> FlipSign() => new(OperationType.FlipSign, false);
    /// <summary> Create a new step taking the reciprocal of the chain's current value. Unsupported for integral types. </summary>
    public static OpChainStep<T> Reciprocal() => new(OperationType.Reciprocal, false);
